...
{
  /// BTP Type @ref eETSIGeoNetNH
  uint8_t BTPType;
  /// GN Packet Transport type @ref eETSIGeoNetTransport
  uint8_t PktTransport;
  /// GN Traffic Class @ref tETSIGeoNetTC
  ...
  /// GN Communication Profile @sa eETSIGeoNetProfile
  uint8_t CommProfile;
  /// GN Repetition Interval (unsupported)
  uint8_t RepeatInterval;

  /// GN Security Type @sa eUDPBTPDataSecType
  uint8_t SecProfile;
...
typedef struct UDPBTPDataIndHdr
{
  ...
  /// Result of Security Verification @sa eC2XSEC_VerificationResult
